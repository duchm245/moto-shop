...
import com.motoshop.config.ZaloPayConfig;
import com.motoshop.models.OrderItem;
import com.motoshop.models.Orders;
import com.motoshop.repositories.OrdersRepository;
import com.motoshop.services.OrdersService;
import com.motoshop.utils.HMACUtil;
...
public class ZaloPayPaymentController {
    ...
    public Map<String, Object> createPayment(@RequestParam(name = "orderId") Long orderId) throws Exception {
        ...
        embeddata.put("merchantinfo", "motoshop");
        ...
        columninfo.put("store_name", "MotoShop");
        embeddata.put("columninfo", gson.toJson(columninfo));
        zalopay_Params.put("embeddata", gson.toJson(embeddata));
...
//		zalopay_Params.put("address", order.getAddress());
        ...
        kq.put("returnmessage", result.has("returnmessage") ? result.get("returnmessage").getAsString() : null);
        kq.put("orderurl", result.has("orderurl") ? result.get("orderurl").getAsString() : null);
        kq.put("returncode", result.has("returncode") ? result.get("returncode").getAsString() : null);
        kq.put("zptranstoken", result.has("zptranstoken") ? result.get("zptranstoken").getAsString() : null);
        return kq;
    }
    ...
    public Map<String, Object> getStatusByApptransid(@RequestParam(name = "apptransid") String apptransid) throws Exception {
        ...
        kq.put("returncode", result.has("returncode") ? result.get("returncode").getAsString() : null);
        kq.put("returnmessage", result.has("returnmessage") ? result.get("returnmessage").getAsString() : null);
        kq.put("isprocessing", result.has("isprocessing") ? result.get("isprocessing").getAsString() : null);
        kq.put("amount", result.has("amount") ? result.get("amount").getAsString() : null);
        kq.put("discountamount", result.has("discountamount") ? result.get("discountamount").getAsString() : null);
        kq.put("zptransid", result.has("zptransid") ? result.get("zptransid").getAsString() : null);
        // Cập nhật trạng thái isCheckout và hình thức thanh toán
        if (result.has("returncode") && "1".equals(result.get("returncode").getAsString())) {
