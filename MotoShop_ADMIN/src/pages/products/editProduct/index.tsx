...
const EditProduct = () => {
  ...
  React.useEffect(() => {
    ...
    setVehicleCondition(product.vehicleCondition || 'new');
    setManufacturingYear(product.manufacturingYear ? String(product.manufacturingYear) : '');
    setMileage(String(product.mileage || 0));
    setBrand(product.brand || '');
    setVehicleType(product.vehicleType || '');
    setEngineType(product.engineType || '');
    setDisplacement(product.displacement ? String(product.displacement) : '');
    setMaxPower(product.maxPower || '');
    setMaxTorque(product.maxTorque || '');
    setTransmission(product.transmission || '');
    setFuelSystem(product.fuelSystem || '');
    setFuelCapacity(product.fuelCapacity ? String(product.fuelCapacity) : '');
    setFuelConsumption(product.fuelConsumption || '');
    setDimensions(product.dimensions || '');
    setWeight(product.weight ? String(product.weight) : '');
    setSeatHeight(product.seatHeight ? String(product.seatHeight) : '');
    setGroundClearance(product.groundClearance ? String(product.groundClearance) : '');
    setWarrantyInfo(product.warrantyInfo || '');
    setOrigin(product.origin || '');
    setInstallmentSupported(product.installmentSupported || false);
    setInstallmentMonths(String(product.installmentMonths || 36));
    setDownPaymentPercent(String(product.downPaymentPercent || 30));
    setOldImages(product.images && product.images.length > 0 ? product.images : []);
    if (product.variants && product.variants.length > 0) {
      ...
      setVariants([{ name: '', colorName: '', colorCode: '#000000', stock: '1' }]);
    }
  }, [product]);
  ...
  const onFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (filesFromLocal) {
      ...
      for (let i = 0; i < filesFromLocal.length; i++) {
        if (newSelectedImages.length < 4) newSelectedImages.push(filesFromLocal[i]);
        else break;
      }
  ...
  const handleDeleteImage = async (id: number) => {
    ...
    try {
      ...
      const [res] = await Promise.all([REQUEST_API({ url, method: 'delete', token })]);
      if (res.status) {
  ...
  const getProduct = async () => {
    if (!token) {
      toast.error('Vui lòng đăng nhập lại', { position: 'top-right', pauseOnHover: false, theme: 'dark' });
      return;
    }
    try {
      ...
      <div className="flex items-start justify-around mt-0 gap-4">
        ...
        <div className="w-[25%] flex flex-col gap-4">
          ...
          <div className="flex flex-col border rounded-md p-5">
            ...
            <div className="w-full h-[1px] bg-black mb-3"></div>
            <select
              className="h-9 border-black text-black border-[1px] rounded-lg px-2"
              onChange={handleCategoryChange}
              value={categoryId}
            >
              <option value="">--Chọn danh mục--</option>
              {category.map((item, i) => (
                <option value={item.id} key={i} className="text-black">{item.title}</option>
              ))}
            </select>
          </div>
        </div>
