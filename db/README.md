...
> ⚠️ **LƯU Ý QUAN TRỌNG VỀ ENCODING:** Các file SQL chứa tiếng Việt UTF-8.  
> **KHÔNG** dùng PowerShell `Get-Content ... | docker ...` vì PowerShell tự động convert encoding làm hỏng dấu tiếng Việt trong DB (hiển thị `???`).  
> Hãy dùng **CMD redirect** (`<`) như hướng dẫn bên dưới, hoặc PowerShell với `-Encoding UTF8`.
...
> ⚠️ **Lỗi thường gặp:** Nếu tiếng Việt hiển thị `???` trong DB — xem mục [Sửa lỗi font tiếng Việt](#sửa-lỗi-font-tiếng-việt) phía dưới.
...
> ✅ Chú ý `--default-character-set=utf8mb4` và dùng **CMD redirect** `<` — đây là điểm mấu chốt.
